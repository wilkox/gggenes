...
#' section (only valid when used with `xmin`/`xmax` aesthetics)
#'
#' @section Aesthetics:
#'
...
geom_circular_plasmid_label <- function(
  ...
  if (! variant %in% c("default", "left", "right",
    ...
    cli::cli_abort("Unrecognised value {.val {variant}} for {.arg variant} argument to {.fun geom_circular_plasmid_label}")
  }
...
GeomCircularPlasmidLabel <- ggplot2::ggproto(
  ...
  setup_data = function(data, params) {
    ...
    if (params$variant == "centre" & ! "xmin" %in% names(data)) {
      cli::cli_abort("Cannot use {.val centre} variant without {.val xmin}/{.val xmax} aesthetics in {.fun geom_circular_plasmid_label}") 
    }
  ...
  setup_params = function(data, params) {
    ...
    if (is.null(params$variant)) {
      ...
      cli::cli_abort("{.val {params$variant}} is not a valid value for {.arg variant} in {.fun geom_circular_plasmid_label}")
    }
