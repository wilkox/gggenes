...
`xmin>=xsubmin` and `xmax>=xsubmax` for all subgenes.
...
``` r
example_subgenes$direction <- ifelse(example_subgenes$strand == "forward", 1, -1)
ggplot2::ggplot(example_genes,
  ...
  geom_subgene_arrow(data=example_subgenes,
    ggplot2::aes(xmin = start, xmax = end, y = molecule, fill = gene,
                 forward = direction, xsubmin=from, xsubmax=to), color='black', alpha=.7) +
  theme_genes()
```
