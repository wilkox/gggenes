...
test_that("geom_gene_arrow() and geom_gene_label() in Cartesian coordinates", { 
  p <- base_cartesian_legacy() + geom_gene_arrow(linewidth = 1) + geom_gene_label()
...
test_that("geom_gene_arrow() and geom_gene_label() in flipped coordinates", { 
  p <- base_flipped_legacy() + geom_gene_arrow(linewidth = 1) + geom_gene_label(height = grid::unit(5, "mm"))
  expect_no_error( { print(p) } )
  expect_doppelganger(
...
test_that("geom_gene_arrow() and geom_gene_label() in polar coordinates", { 
  p <- base_polar_legacy() + geom_gene_arrow(linewidth = 1) + geom_gene_label()
  expect_no_error( { print(p)} )
  expect_doppelganger(
