...
is a ‘ggplot2’ geom that represents genes with arrows. The start and end
locations of the genes within their molecule(s) are mapped to the `xmin`
and `xmax` aesthetics respectively. These start and end locations are
used to determine the directions in which the arrows point. The `y`
...
with `scales = "free"` to avoid drawing ridiculously large molecules
...
![](introduction-to-gggenes_files/figure-html/geom_gene_arrow-1.png)

## Beautifying the plot with `theme_genes()`

Because the resulting plot can look cluttered, a ‘ggplot2’ theme
[`theme_genes()`](https://wilkox.org/gggenes/reference/theme_genes.md)
...
![](introduction-to-gggenes_files/figure-html/theme_genes-1.png)
...
faceted molecules.
[`make_alignment_dummies()`](https://wilkox.org/gggenes/reference/make_alignment_dummies.md)
generates a set of ‘dummy’ genes that if added to the plot with
[`ggplot2::geom_blank()`](https://ggplot2.tidyverse.org/reference/geom_blank.html)
will extend the range of each facet to visually align the selected gene
across facets.
...
![](introduction-to-gggenes_files/figure-html/make_alignment_dummies-1.png)

## Labelling genes with `geom_gene_label()`

To label individual genes, provide a `label` aesthetic and use
[`geom_gene_label()`](https://wilkox.org/gggenes/reference/geom_gene_label.md).
[`geom_gene_label()`](https://wilkox.org/gggenes/reference/geom_gene_label.md)
uses the [‘ggfittext’](https://github.com/wilkox/ggfittext) package to
fit the label text inside the gene arrows; see the ‘ggfittext’
documentation for more details on how it resizes and reflows text to
make it fit.
...
![](introduction-to-gggenes_files/figure-html/labelled_genes-1.png)
...
![](introduction-to-gggenes_files/figure-html/reversing_direction-1.png)

## Viewing subgene segments

We can highlight subgene segments, such as protein domains or local
alignments, using
[`geom_subgene_arrow()`](https://wilkox.org/gggenes/reference/geom_subgene_arrow.md).

This works similarly to
[`geom_gene_arrow()`](https://wilkox.org/gggenes/reference/geom_gene_arrow.md),
but in addition to `xmin` and `xmax` (which determine the gene
boundaries), we need the aesthetics `xsubmin` and `xsubmax` to determine
the subgene boundaries.
[`geom_gene_arrow()`](https://wilkox.org/gggenes/reference/geom_gene_arrow.md)
will produce pretty arrowheads, as long as `xmin >= xsubmin` and
`xmax >= xsubmax` for all subgenes (subgenes that break gene boundaries
will be skipped with a warning).

The suggested usage is to use
[`geom_gene_arrow()`](https://wilkox.org/gggenes/reference/geom_gene_arrow.md)
...
![](introduction-to-gggenes_files/figure-html/subgenes-1.png)

To label subgenes, we can use
[`geom_subgene_label()`](https://wilkox.org/gggenes/reference/geom_subgene_label.md),
which works similarly to
[`geom_gene_label()`](https://wilkox.org/gggenes/reference/geom_gene_label.md)
with the major difference that it requires `xsubmin` and `xsubmax`
...
![](introduction-to-gggenes_files/figure-html/subgene%20labels-1.png)

## Drawing point genetic features

We can draw point genetic features, such as restriction sites or
transcription start sites, with
...
Both of these geoms take an optional `forward` aesthetic to indicate
whether a feature is oriented and, if so, in what direction (`TRUE`
meaning oriented towards the right and `FALSE` meaning towards the
left).
...
![](introduction-to-gggenes_files/figure-html/unnamed-chunk-2-1.png)
