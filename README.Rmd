...
by using the subgene_arrow geom.

This works similarly to `geom_gene_arrow`, but in addition to `xmin` and `xmax`
(which determines the gene boundary), we need the aesthetics `xsubmin` and
`xsubmax`, which will produce pretty arrowheads as long as `xmin>=xsubmin` and
`xmax>=xsubmax` for all subgenes.

The suggested usage is to use geom_gene_arrow with no fill, and then add a
subgene layer over this. This example uses randomly drawn segments:
