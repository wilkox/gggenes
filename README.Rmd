...
Often you might want a certain gene to be vertically aligned across the faceted
molecules. A helper function `make_alignment_dummies` is provided that generates
a set of 'dummy' genes such that, if the dummies are added to the plot with
`geom_blank`, they will extend the range of each facet to visually align the
selected gene across facets.
...
To label individual genes, set the `label` aesthetic and use `geom_gene_label`.
`geom_gene_label` uses the ['ggfittext'](https://github.com/wilkox/ggfittext)
package to fit the label text inside the gene arrows; see the 'ggfittext'
documentation for more details on how it resizes and reflows text to make it
fit.
